...
void loop() {
  if (millis() - lastSendTime > interval) {
    ...
    sendMessage(cryptmsgstr);
    Serial.println(cryptmsgstr.length());
    ...
    byte outputtext[cryptmsgstr.length()];
    byte buf[cryptmsgstr.length()];

    cryptmsgstr.getBytes(buf, cryptmsgstr.length());

    chacha2.decrypt(outputtext,buf,cryptSize);

    Serial.println((char*)outputtext);
